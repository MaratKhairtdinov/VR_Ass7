...
#        self.navidget_node.Transform.value = avango.gua.make_identity_mat()
        ...
        self.sphere_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(1.0,1.0,1.0,0.03))

        self.sphere_geometry.Tags.value = ["invisible"]
        self.navidget_node.Children.value.append(self.sphere_geometry)

        self.small_ray_geometry = _loader.create_geometry_from_file("ray_geometry", "data/objects/cylinder.obj", avango.gua.LoaderFlags.DEFAULTS)
        self.small_ray_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(0.0,0.0,1.0,1.0))
        self.small_ray_geometry.Tags.value = ["invisible"]

        self.navidget_node.Children.value.append(self.small_ray_geometry)
        ...
        self.navidget_node.Children.value.append(self.camera_geometry)
...
class TeleportNavigation(NavigationTechnique):
    ...
    def evaluate(self): # implement respective base-class function
        ...
        if(self.sf_pointer_button.value == False and self.flag == True):
            self.flag = False
            self.teleportation_matrix = avango.gua.make_trans_mat(self.hit_point)
            #nav_matrix = self.NAVIGATION_MANAGER.get_navigation_matrix()
            offset_x = -self.NAVIGATION_MANAGER.get_head_matrix().get_translate().x
            offset_z = -self.NAVIGATION_MANAGER.get_head_matrix().get_translate().z
            offset = avango.gua.make_trans_mat(offset_x, 0, offset_z)

            self.NAVIGATION_MANAGER.set_navigation_matrix(self.teleportation_matrix*offset)
            self.teleportation_matrix = None            
...
class NavidgetNavigation(NavigationTechnique):
    ...
    def my_constructor(self, NAVIGATION_MANAGER, SCENEGRAPH):
        ...
        self.ray_length = 25.0
        self.ray_thickness = 0.015
    ...
    def evaluate(self): # implement respective base-class function
        if self.enable_flag == False:            
            self.NAVIGATION_MANAGER.sphere_geometry.Tags.value = ["invisible"]
            self.NAVIGATION_MANAGER.small_ray_geometry.Tags.value = ["invisible"]
        ...
        if(self.sf_pointer_button.value == True and self.flag == False):
            self.flag = True
            self.NAVIGATION_MANAGER.sphere_geometry.Tags.value = []
            self.NAVIGATION_MANAGER.small_ray_geometry.Tags.value = []
            self.NAVIGATION_MANAGER.camera_geometry.Tags.value = []   

            self.hit_point = avango.gua.make_trans_mat(self.NAVIGATION_MANAGER.intersection_geometry.WorldTransform.value.get_translate())

            self.init_translate = self.NAVIGATION_MANAGER.pointer_node.Transform.value * avango.gua.Vec4(0,0,0,1)


            #self.NAVIGATION_MANAGER.navidget_node.WorldTransform.value = self.hit_point
        ...
        if (self.sf_pointer_button.value == False):
            self.NAVIGATION_MANAGER.ray_geometry.Tags.value = []
            self.NAVIGATION_MANAGER.intersection_geometry.Tags.value = []
        ...
        if (self.sf_pointer_button.value == True):
            ...
            z = self.NAVIGATION_MANAGER.pointer_node.WorldTransform.value.get_translate().z-self.init_translate.z# roll 
            y = self.NAVIGATION_MANAGER.pointer_node.WorldTransform.value.get_translate().y-self.init_translate.y# pitch
            x = self.NAVIGATION_MANAGER.pointer_node.WorldTransform.value.get_translate().x-self.init_translate.x# yaw
            
            roll = self.NAVIGATION_MANAGER.pointer_node.WorldTransform.value.get_rotate().z
            pitch = self.NAVIGATION_MANAGER.pointer_node.WorldTransform.value.get_rotate().x
            yaw = self.NAVIGATION_MANAGER.pointer_node.WorldTransform.value.get_rotate().y

            print(roll, pitch, yaw)
  
            self.NAVIGATION_MANAGER.navidget_node.Transform.value = self.hit_point * avango.gua.make_rot_mat(x*360, 0,1,0)
            self.NAVIGATION_MANAGER.small_ray_geometry.Transform.value = avango.gua.make_rot_mat(y*180, 1,0,0) * avango.gua.make_scale_mat(self.ray_thickness, z*25, self.ray_thickness)       #avango.gua.make_scale_mat(self.ray_thickness, roll, self.ray_thickness) * avango.gua.make_rot_mat(pitch*180, 0,0,1)         
                #avango.gua.make_trans_mat(0.0,0.0,_distance * -0.5) * \
#                avango.gua.make_rot_mat(-90.0,1,0,0) * \
